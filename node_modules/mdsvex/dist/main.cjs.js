...
const open_tag$1 = '<[A-Za-z][A-Za-z0-9\\-]*:*[a-zA-Z]+' + attribute$1 + '*\\s*\\/?>';
const close_tag$1 = '<\\/[A-Za-z][A-Za-z0-9\\-]*\\s*>';
const HTML_OPEN_CLOSE_TAG_RE$2 = new RegExp('^(?:' + open_tag$1 + '|' + close_tag$1 + ')');
...
const svelteBlock = [
    ...
    [new RegExp(HTML_OPEN_CLOSE_TAG_RE$2.source), /^$/, false],
];
...
function codeExec(md) {
    ...
    md.renderer.rules['fence'] = function (tokens, idx, options, env, self) {
        ...
        if ((css_lang = tokens[idx].info.match(/^([a-z]+) style$/))) {
            ...
            if (md.svxstyles[0].length === 0)
                md.svxstyles[0] = css_lang[1];
...
var regex$5 = new RegExp(pattern, 'm');
...
function test (string) {
  ...
  return regex$5.test(string)
}
frontMatter.test = test_1;

const extname = pathBrowserify.extname;
const defaultOpts = {
    parser: md => md,
    markdownOptions: {},
    extension: '.svexy',
};
function mdsvex({ parser = md => md, markdownOptions = {}, extension = '.svexy', layout, } = defaultOpts) {
        ...
        markup: ({ content, filename }) => {
            ...
            if (isAttributes || isExec || isMod) {
                ...
                if (isMod || isAttributes) {
                    ...
                    if (isAttributes) {
                        modules += `export const _metadata = ${JSON.stringify(attributes)};`;
                    }
